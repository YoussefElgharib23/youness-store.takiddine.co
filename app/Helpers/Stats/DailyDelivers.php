...
class DailyDelivers { 
    ...
    public function canceled() {
       $this->init();
       $this->canceled = $this->listing->whereNotNull('canceled_at')->where('statue','!=','NoAnswer')->whereNull('delivred_at')->whereNull('recall_at')->whereDate('created_at', '=', $this->day )->count();
    }


    public function noanswer() {
       $this->init();
       $this->noanswer = $this->listing->whereNull('canceled_at')->where('statue','NoAnswer')->whereNull('delivred_at')->whereDate('created_at', '=', $this->day )->count();
    }


    public function recall() {
        $this->init();
        $this->recall =  $this->listing->whereNull('canceled_at')->whereNull('delivred_at')->where('statue','!=','NoAnswer')->whereNotNull('recall_at')->whereDate('created_at', '=', $this->day )->count();
    }
 
 
    public function delivred() {
       $this->delivred = \App\Models\Lists::where('DeliverID',$this->deliver)->whereNotNull('delivred_at')->whereDate('created_at', '=', $this->day )->count();
    }
